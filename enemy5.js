function Enemy5(x, y) {
    ...
    this.h = 15
    this.hp = 1
    this.delta = 4 // move speed of enemy
    this.col = "#F533FF"
    this.str = "#F533FF"
    this.coll1
    this.coll2
...
Enemy5.prototype.move = function () {
    ...
    Enemy5.prototype.attack = function () {
        var p1 = {
            x: player.x,
            y: player.y
        };
        ...
        if (player.hp > 0) {
            ...
            if (this.frames < 2) {
                ...
                this.y += Math.random()
                this.y -= Math.random()
            } else if (this.frames > 2 && this.frames < 30) {
    ...
    Enemy5.prototype.show = function () {
        ellipse(this.x, this.y, this.h)

        stroke("#f533ff") // health bar for enemy
        fill("#BD1E6E")
        rect(this.x - 7, this.y + 19.25, 13.5, 13)
        fill("#f533ff")
        for (var i = 0; i < this.hp; i++) {
            rect(this.x - 3.5 + 9 * i, this.y + 23, 6, 6)
        }

    }
}

Enemy5.prototype.wallcoll = function () { //detects if object is touching wall
...
Enemy5.prototype.collDetect = function (playerObj) { // detects if object is touching player
...
Enemy5.prototype.hitDetect = function () { // detects if object is hit by player
